@foreach ($products as $product)
    @php
        ...
        $availabilityLabel = $isUpdating
            ? 'Checkout paused Â· Discord alerts'
            : ($stock > 0 ? $stock.' available Â· Auto delivery' : 'Manual order via Discord');
    ...
    <a href="{{ route('products.show', $product) }}" data-soft-nav data-scroll-reveal
        data-home-reveal-item data-home-reveal-stage="product-card"
        class="product-card product-card-storefront {{ $isUpdating ? 'product-card-is-updating' : '' }} flex min-h-60 flex-col gap-4 p-5"
        ...
        <div>
            ...
            <p class="mt-2 line-clamp-3 text-sm leading-6 text-gray-400">{{ $product->description }}</p>
        </div>
        ...
        <div class="mt-auto flex items-end justify-between gap-4">
            <div>
                <div class="mb-1 text-[10px] uppercase tracking-wide text-gray-500">Start from</div>
